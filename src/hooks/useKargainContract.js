...
const useKargainContract = () => {
    ...
    const { web3: library, address: account } = useWeb3Modal()
    ...
    const fetchPlatformPercent = useCallback(async () => {
        try {
            ...
            const value = await contract.methods.platformCommissionPercent().call()
    ...
    const watchOfferEvent = useCallback(
        async (tokenId) => {
            try {
    ...
    const watchOfferRejected = useCallback(
        async (tokenId) => {
            try {
                if (!contract || !library) return
    ...
    const watchOfferAccepted = useCallback(
        async (tokenId) => {
            try {
                if (!contract || !library) return
    ...
    const updatePlatformPercent = useCallback(
        async (percent) => {
            try {
                if (!contract || !library) return
                ...
                const tx = await contract.methods.setPlatformCommissionPercent(percent).send({ from: account })
    ...
    const fetchOfferExpirationTime = useCallback(async () => {
        try {
            ...
            const value = await contract.methods.offerExpirationTime().call()
    ...
    const updateOfferExpirationTime = useCallback(
        async (days) => {
            try {
                if (!contract || !library) return
                ...
                const tx = await contract.methods.setOfferExpirationTime(days * ONE_DAY).send({ from: account })
    ...
    const waitTransactionToBeConfirmed = useCallback(
    ...
    const rejectOffer = useCallback(
        async (tokenId) => {
            try {
                ...
                const tx = await contract.methods.rejectOffer(tokenId).send({ from: account })
    ...
    const mintToken = useCallback(
        async (tokenId, price) => {
            try {
                ...
                const tx = await contract.methods.mint(tokenId, waiPrice).send({ from: account })
    ...
    const updateTokenPrince = useCallback(
        async (tokenId, price) => {
            try {
                ...
                const tx = await contract.methods.setTokenPrice(tokenId, waiPrice).send({ from: account })
